...
- Handle exceptions
- Client stuff
- Silently reject corrupted records at end of queue
- Otherwise, nuke disk cache of corrupted queue, sync back to lastCounter
- Tests for client code using mock server
...
function summon (queueName) {
  ...
  events[queueName] = Geval(function (broadcast) {
...
function syncOne (syncData) {
  ...
  R.forEach(function (rec) {
    ...
    if (rec.i !== queue.length) throw new E.GapError()
    save(queueName, rec)
  }, recs)
}
